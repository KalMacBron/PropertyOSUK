...
class _SignInScreenState extends ConsumerState<SignInScreen> {
  ...
  Future<void> _signIn() async {
    ...
    try {
      await AuthRepository(ref.read(supabaseProvider)).signIn(email: _email.text, password: _password.text);
    } catch (_) {
        ...
        child: Card(
          child: Padding(
            ...
            child: Form(key: _formKey, child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              const Text('PropertyOS', style: TextStyle(fontSize: 30, fontWeight: FontWeight.w600)),
              ...
              SizedBox(width: double.infinity, child: FilledButton(onPressed: _submitting ? null : _signIn, child: Text(_submitting ? 'Signing in…' : 'Sign in'))),
              const SizedBox(height: 16),
              const Text('Access is invitation-only during the Alpha.', style: TextStyle(fontSize: 12)),
            ])),
          ),
        ),
      ),
